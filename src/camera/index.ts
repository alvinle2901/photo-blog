...
export function absolutePathForPhoto(make: string, model: string, photoId: string): string {
  return `/shot-on/${encodeURIComponent(make)}/${encodeURIComponent(model)}/${photoId}`;
}

export function absolutePathForCameraImage(make: string, model: string): string {
  return `/shot-on/${encodeURIComponent(make)}/${encodeURIComponent(model)}/image`;
}

export function titleForCamera(make: string, model: string, count: number): string {
  const label = cameraLabel(make, model);
  return `${label} (${count} ${count === 1 ? 'photo' : 'photos'})`;
}

export function descriptionForCamera(make: string, model: string, count: number): string {
  const label = cameraLabel(make, model);
