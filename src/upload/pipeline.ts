...
import { type Photo, rowToPhoto } from "../photo";
...
export async function processUpload(input: UploadInput): Promise<Photo> {
	...
	const [row] = await db
		...
		.values({
			...
			model: exif.model,
			focalLength:
				exif.focalLength != null ? Math.round(exif.focalLength) : null,
			focalLength35mm:
				exif.focalLength35mm != null ? Math.round(exif.focalLength35mm) : null,
			fStop: exif.fStop,
